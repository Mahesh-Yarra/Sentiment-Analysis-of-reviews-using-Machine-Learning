...
# Paths for files
LEXICON_POSITIVE = r"Backend/Datasets/positive-words.txt"
LEXICON_NEGATIVE = r"Backend/Datasets/negative-words.txt"
LEXICON_CONNOTATION = r"Backend/Datasets/connotations.csv"

# Load the trained model and necessary preprocessing steps
# Load the saved model
model = load_model('Backend/Models/sentiment_model.h5')
vectorizer = joblib.load('Backend/Models/tfidf_vectorizer.pkl')
selector = joblib.load('Backend/Models/correlated_feature_picker.pkl')
lda_pipeline = joblib.load('Backend/Models/lda_topics_extractor.pkl')
...
def predict():
    ...
    # Scale features
    selected_features.columns = selected_features.columns.astype(str)  # Convert feature names to strings
    X_scaled = scale_features(selected_features)
    ...
    X_scaled_dict = X_scaled.to_dict(orient='records')
    prediction_list.append(X_scaled_dict)
    # Return prediction result
    return jsonify({'sentiment': prediction_list})
